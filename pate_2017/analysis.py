...
from privacy.research.pate_2017.input import maybe_download
...
def perform_analysis(
	...
	"""
	# paper_binaries_mnist = ["https://github.com/npapernot/multiple-teachers-for-privacy/blob/master/mnist_250_teachers_labels.npy?raw=true", "https://github.com/npapernot/multiple-teachers-for-privacy/blob/master/mnist_250_teachers_100_indices_used_by_student.npy?raw=true"]
	# if FLAGS.counts_file == "mnist_250_teachers_labels.npy" or FLAGS.indices_file == "mnist_250_teachers_100_indices_used_by_student.npy":
	# 	maybe_download(paper_binaries_mnist, os.getcwd())

	# Binaries for SVHN results
	# FLAGS.counts_file == "svhn_250_teachers_labels.npy"
	...
	# teacher_preds = np.load("mnist_250_teachers_labels.npy")
	# if FLAGS.input_is_counts:
		# counts_mat = teacher_preds
	# else:
	# In this case, the input is the raw predictions. Transform
	# print(teacher_preds.shape)
	# print(teacher_preds[0])
	teacher_preds = np.transpose(teacher_preds)
	num_teachers, n = teacher_preds.shape
	counts_mat = np.zeros((n, num_classes)).astype(np.int32)
	for i in range(n):
	...
	# 	print("0021")
	# else:
	# index_list = np.load("mnist_250_teachers_100_indices_used_by_student.npy")
