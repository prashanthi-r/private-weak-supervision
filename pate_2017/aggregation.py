...
def noisy_max(logits, lap_scale, return_clean_votes=False, num_classes=10):
  """
  ...
  # print("\nfrom noisy max: ",labels.shape)
  # labels_shape = np.shape(labels)
  # labels = labels.reshape((labels_shape[0], labels_shape[1]))

  L = np.array(logits) # added by Prashanthi to avoid any confusion with the variable name "logits"
  # Initialize array to hold final labels
  L = np.transpose(L)
  labels_shape = np.shape(L)
  result = []

  if return_clean_votes:
    # Initialize array to hold clean votes for each sample
    # added by shashi and prashi to accommodate a general number of classes
    clean_votes = np.zeros((int(labels_shape[1]), num_classes))

  # Parse each sample
  for i in range(int(labels_shape[1])):
    # Count number of votes assigned to each class
    if np.all(L[:, i] > -1):
        # print("found no abstains")
        label_counts = np.bincount(L[:, i], minlength=num_classes)
    else:
        # print("Found some abstains here hehe :)")
        label_counts = [0]*num_classes
        label_counter = Counter(L[:,i])
        del label_counter[ABSTAIN]
        for i in range(num_classes):
            if i in label_counter:
                label_counts[i] = label_counter[i]
            else:
                label_counts[i] = 0
    ...
    print("float label_counts: ", label_counts)
    ...
    print("after adding noise: ", label_counts)
    # Result is the most frequent label
    result.append(np.argmax(label_counts))
...
def aggregation_most_frequent(logits):
  ...
  for i in xrange(int(labels_shape[1])):
    # Count number of votes assigned to each class
    label_counts = np.bincount(labels[:, i], minlength=num_classes)
